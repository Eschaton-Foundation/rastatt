(function () {
	...
	***************************************************************************** */
	...
	var __createBinding$2 = Object.create
		? function (o, m, k, k2) {
	...
	var __setModuleDefault$2 = Object.create
		? function (o, v) {
	...
	function flatten(options) {
		return (options === null || options === void 0 ? void 0 : options.xml)
			? typeof options.xml === "boolean"
				? xmlModeDefault
				: tslib_1$6.__assign(
						tslib_1$6.__assign({}, xmlModeDefault),
						options.xml
				  )
	...
	Object.defineProperty(parse$8, "__esModule", { value: true });
	parse$8.isTraversal = void 0;
	...
	parse$8.isTraversal = isTraversal;
	 ...
	 */
	function parse$7(selector, options) {
		var subselects = [];
		...
		while (selector !== "") {
			...
			if (isWhitespace$1(firstChar)) {
				...
				if (firstChar in attribSelectors) {
					...
					var action = "exists";
					var possibleAction = actionTypes$1.get(
						selector.charAt(selectorIndex)
					);
					if (possibleAction) {
					...
					if (selector.charAt(selectorIndex) === "(") {
						if (unpackPseudos.has(name_3)) {
							if (
								quotes.has(selector.charAt(selectorIndex + 1))
							) {
								throw new Error(
									"Pseudo-selector " +
										name_3 +
	...
	var Node$1 = /** @class */ (function () {
		...
		Object.defineProperty(Node.prototype, "nodeType", {
			...
			get: function () {
				var _a;
				return (_a = nodeTypes$1.get(this.type)) !== null &&
					_a !== void 0
					? _a
					: 1;
			},
	...
	var Text = /** @class */ (function (_super) {
		...
		function Text(data) {
			return (
				_super.call(this, domelementtype_1$1.ElementType.Text, data) ||
				this
			);
		}
		return Text;
	...
	var Comment = /** @class */ (function (_super) {
		...
		function Comment(data) {
			return (
				_super.call(
					this,
					domelementtype_1$1.ElementType.Comment,
					data
				) || this
			);
		}
	...
	var ProcessingInstruction = /** @class */ (function (_super) {
		...
		function ProcessingInstruction(name, data) {
			var _this =
				_super.call(
					this,
					domelementtype_1$1.ElementType.Directive,
					data
				) || this;
			_this.name = name;
			return _this;
	...
	var Document = /** @class */ (function (_super) {
		...
		function Document(children) {
			return (
				_super.call(
					this,
					domelementtype_1$1.ElementType.Root,
					children
				) || this
			);
		}
			...
			get: function () {
				...
				return Object.keys(this.attribs).map(function (name) {
					...
					return {
						...
						namespace:
							(_a = _this["x-attribsNamespace"]) === null ||
							_a === void 0
								? void 0
								: _a[name],
						prefix:
							(_b = _this["x-attribsPrefix"]) === null ||
							_b === void 0
								? void 0
								: _b[name],
					};
	...
	function cloneNode(node, recursive) {
		...
		if (isText(node)) {
			...
			if (node["x-attribsPrefix"]) {
				clone_1["x-attribsPrefix"] = __assign$1(
					{},
		...
		var DomHandler = /** @class */ (function () {
			...
			DomHandler.prototype.onopentag = function (name, attribs) {
				var type = this.options.xmlMode
					...
					: undefined;
				var element = new node_1.Element(
					name,
					attribs,
					undefined,
					type
		...
		return function replace(str) {
			if (str.charAt(1) === "#") {
				...
				if (secondChar === "X" || secondChar === "x") {
					return decode_codepoint_1$1.default(
		...
		return Object.keys(attributes)
			.map(function (key) {
				...
				if (opts.xmlMode === "foreign") {
					...
					key =
						(_b = foreignNames_1.attributeNames.get(key)) !==
							null && _b !== void 0
							? _b
							: key;
				}
	...
	function getText(node) {
		...
		if ((0, domhandler_1$a.isCDATA)(node)) return getText(node.children);
		if ((0, domhandler_1$a.isText)(node)) return node.data;
		return "";
	}
	...
	function textContent(node) {
		...
		if (
			(0, domhandler_1$a.hasChildren)(node) &&
			!(0, domhandler_1$a.isComment)(node)
		) {
	...
	manipulation$1.prepend =
		manipulation$1.prependChild =
		manipulation$1.append =
		manipulation$1.appendChild =
		manipulation$1.replaceElement =
		manipulation$1.removeElement =
			void 0;
	/**
	...
	function find$1(test, nodes, recurse, limit) {
		...
		for (var _i = 0, nodes_1 = nodes; _i < nodes_1.length; _i++) {
			...
			if (
				recurse &&
				(0, domhandler_1$8.hasChildren)(elem) &&
				elem.children.length > 0
			) {
	...
	function existsOne(test, nodes) {
		return nodes.some(function (checked) {
			return (
				(0, domhandler_1$8.isTag)(checked) &&
				(test(checked) ||
		...
		while ((elem = stack.shift())) {
			var children =
				(_a = elem.children) === null || _a === void 0
					? void 0
					: _a.filter(domhandler_1$8.isTag);
	...
	var Checks = {
		tag_name: function (name) {
			if (typeof name === "function") {
				return function (elem) {
					return (0, domhandler_1$7.isTag)(elem) && name(elem.name);
				};
			} else if (name === "*") {
				return domhandler_1$7.isTag;
			}
			return function (elem) {
				return (0, domhandler_1$7.isTag)(elem) && elem.name === name;
			};
		},
		...
		tag_contains: function (data) {
			if (typeof data === "function") {
				return function (elem) {
					return (0, domhandler_1$7.isText)(elem) && data(elem.data);
				};
			}
			return function (elem) {
				return (0, domhandler_1$7.isText)(elem) && elem.data === data;
			};
		},
	};
	/**
	...
	helpers$1.uniqueSort =
		helpers$1.compareDocumentPosition =
		helpers$1.removeSubsets =
			void 0;
	...
	function compareDocumentPosition(nodeA, nodeB) {
		...
		var current = (0, domhandler_1$6.hasChildren)(nodeA)
			? nodeA
			: nodeA.parent;
		while (current) {
		...
		var feed = {
			...
			items: (0, legacy_1.getElementsByTagName)("entry", childs).map(
				function (item) {
					...
					var href =
						(_a = getOneElement$1("link", children)) === null ||
						_a === void 0
							? void 0
							: _a.attribs.href;
					if (href) {
	...
	attributes$1.attributeRules = {
		equals: function (next, data, _a) {
			...
			return function (elem) {
				return (
					adapter.getAttributeValue(elem, name) === value &&
					next(elem)
				);
			};
		},
		...
		start: function (next, data, _a) {
			...
			if (data.ignoreCase) {
				...
				return function (elem) {
					var _a;
					return (
						((_a = adapter.getAttributeValue(elem, name)) ===
							null || _a === void 0
							? void 0
							: _a.substr(len).toLowerCase()) === value &&
						next(elem)
					);
				};
			}
		...
		exports.filters = {
			...
			root: function (next, _rule, _a) {
				...
				return function (elem) {
					var parent = adapter.getParent(elem);
					return (
						(parent == null || !adapter.isTag(parent)) && next(elem)
					);
				};
			},
		...
		exports.subselects = {
			...
			has: function (next, subselect, options, _context, compileToken) {
				...
				if (compiled === boolbase_1.trueFunc) {
					return function (elem) {
						return (
							adapter.getChildren(elem).some(adapter.isTag) &&
							next(elem)
						);
					};
				}
				...
				if (context) {
					return function (elem) {
						...
						var nextElements = shouldTestNextSiblings
							? __spreadArray(
									__spreadArray([], childs),
									getNextSiblings(elem, adapter)
							  )
							: childs;
						return (
							next(elem) &&
							adapter.existsOne(hasElement, nextElements)
						);
					};
				}
				return function (elem) {
	...
	(function (exports) {
		...
		function compilePseudoSelector(
			...
			if (name in aliases_1.aliases) {
				if (data != null) {
					throw new Error(
						"Pseudo " + name + " doesn't have any arguments"
					);
				}
		...
		switch (selector.type) {
			...
			case "tag":
				return function tag(elem) {
					return (
						adapter.getName(elem) === selector.name && next(elem)
					);
				};
			// Traversal
			case "descendant":
			...
			case "child":
				return function child(elem) {
					var parent = adapter.getParent(elem);
					return (
						parent != null && adapter.isTag(parent) && next(parent)
					);
				};
			case "sibling":
	...
	compile$3.compileToken =
		compile$3.compileUnsafe =
		compile$3.compile =
			void 0;
	var css_what_1 = lib$a;
	var boolbase_1 = boolbase;
	...
	(function (exports) {
		...
		function appendNextSiblings(elem, adapter) {
			...
			for (var i = 0; i < elems.length; i++) {
				var nextSiblings = subselects_1.getNextSiblings(
					elems[i],
					adapter
				);
		...
		exports.selectAll = getSelectorFunc(function (query, elems, options) {
			return query === boolbase_1.falseFunc ||
				!elems ||
				elems.length === 0
				? []
	...
	(function (exports) {
		...
		function some(elements, selector, options) {
			...
			return (
				(plain.length > 0 &&
					elements.some(
						css_select_1._compileToken(plain, options)
					)) ||
			...
			switch (filter) {
				...
				case "not": {
					var filtered_1 = new Set(
						filterParsed(data, elems, options)
					);
		...
		function findFilterElements(
			...
			var elemsNoLimit =
				sub.length === 0 && !Array.isArray(root)
					...
					? (Array.isArray(root) ? root : [root]).filter(
							DomUtils.isTag
					  )
		...
		Parser.prototype.updatePosition = function (initialOffset) {
			if (this.endIndex === null) {
				if (this.tokenizer.sectionStart <= initialOffset) {
					...
					this.startIndex =
						this.tokenizer.sectionStart - initialOffset;
				}
			} else {
	...
	var namedEntityData = new Uint16Array([
		...
		113, 117, 97, 108, 59, 1, 8931, 4, 3, 98, 99, 112, 3981, 4000, 4045,
		115, 101, 116, 4, 2, 59, 69, 3990, 3993, 3, 8834, 8402, 113, 117, 97,
		108, 59, 1, 8840, 99, 101, 101, 100, 115, 4, 4, 59, 69, 83, 84, 4015,
		4017, 4025, 4037, 1, 8833, 113, 117, 97, 108, 59, 3, 10928, 824, 108,
		97, 110, 116, 69, 113, 117, 97, 108, 59, 1, 8929, 105, 108, 100, 101,
		59, 3, 8831, 824, 101, 114, 115, 101, 116, 4, 2, 59, 69, 4056, 4059, 3,
		8835, 8402, 113, 117, 97, 108, 59, 1, 8841, 105, 108, 100, 101, 4, 4,
		59, 69, 70, 84, 4080, 4082, 4089, 4100, 1, 8769, 113, 117, 97, 108, 59,
		1, 8772, 117, 108, 108, 69, 113, 117, 97, 108, 59, 1, 8775, 105, 108,
		100, 101, 59, 1, 8777, 101, 114, 116, 105, 99, 97, 108, 66, 97, 114, 59,
		1, 8740, 99, 114, 59, 3, 55349, 56489, 105, 108, 100, 101, 5, 209, 1,
		59, 4135, 1, 209, 59, 1, 925, 4, 14, 69, 97, 99, 100, 102, 103, 109,
		111, 112, 114, 115, 116, 117, 118, 4170, 4176, 4187, 4205, 4212, 4217,
		10062, 10102, 10114, 10135, 10139, 10151, 4, 6, 50, 51, 52, 53, 54, 56,
		10076, 10083, 10086, 10093, 10096, 10099, 5, 189, 1, 59, 10081, 1, 189,
		59, 1, 8531, 5, 188, 1, 59, 10091, 1, 188, 59, 1, 8533, 59, 1, 8537, 59,
		1, 8539, 4, 2, 51, 53, 10108, 10111, 59, 1, 8532, 59, 1, 8534, 4, 3, 52,
		...
		59, 12983, 1, 183, 117, 115, 4, 3, 59, 98, 100, 12995, 12997, 13000, 1,
	...
	class Tokenizer$4 {
		...
		_isCharacterReferenceAttributeQuirk(withSemicolon) {
			if (!withSemicolon && this._isCharacterReferenceInAttribute()) {
				...
				return (
					nextCp === $$7.EQUALS_SIGN || isAsciiAlphaNumeric(nextCp)
				);
			}
		...
		[AFTER_DOCTYPE_PUBLIC_IDENTIFIER_STATE](cp) {
			if (isWhitespace(cp)) {
				...
				this._err(
					ERR$1.missingWhitespaceBetweenDoctypePublicAndSystemIdentifiers
				);
		...
		[AFTER_DOCTYPE_SYSTEM_IDENTIFIER_STATE](cp) {
			...
			if (cp === $$7.GREATER_THAN_SIGN) {
				...
				this._err(
					ERR$1.unexpectedCharacterAfterDoctypeSystemIdentifier
				);
				this._reconsumeInState(BOGUS_DOCTYPE_STATE);
			}
		}
		...
		[NAMED_CHARACTER_REFERENCE_STATE](cp) {
			...
			if (this._ensureHibernation()) {
				...
				const withSemicolon =
					this.tempBuff[this.tempBuff.length - 1] === $$7.SEMICOLON;

				if (!this._isCharacterReferenceAttributeQuirk(withSemicolon)) {
					if (!withSemicolon) {
						this._errOnNextCodePoint(
							ERR$1.missingSemicolonAfterCharacterReference
						);
					}
		...
		[NUMERIC_CHARACTER_REFERENCE_END_STATE]() {
			if (this.charRefCode === $$7.NULL) {
				...
				unicode$1.isControlCodePoint(this.charRefCode) ||
				this.charRefCode === $$7.CARRIAGE_RETURN
			) {
	...
	function isScopingElement(tn, ns) {
		switch (tn.length) {
			...
			case 6:
				return (
					(tn === $$5.APPLET || tn === $$5.OBJECT) && ns === NS$3.HTML
				);

			case 7:
				return (
					(tn === $$5.CAPTION || tn === $$5.MARQUEE) &&
					ns === NS$3.HTML
				);

			case 8:
				return tn === $$5.TEMPLATE && ns === NS$3.HTML;

			case 13:
				return tn === $$5.FOREIGN_OBJECT && ns === NS$3.SVG;

			case 14:
				return tn === $$5.ANNOTATION_XML && ns === NS$3.MATHML;
		}
	...
	class OpenElementStack$1 {
		...
		_isInTemplate() {
			return (
				this.currentTagName === $$5.TEMPLATE &&
		...
		popUntilTableCellPopped() {
			while (this.stackTop > -1) {
				...
				if (tn === $$5.TD || (tn === $$5.TH && ns === NS$3.HTML)) {
					break;
				}
			}
		}
		...
		clearBackToTableContext() {
			while (
				(this.currentTagName !== $$5.TABLE &&
					this.currentTagName !== $$5.TEMPLATE &&
					this.currentTagName !== $$5.HTML) ||
				this.treeAdapter.getNamespaceURI(this.current) !== NS$3.HTML
			) {
				this.pop();
			}
		...
		clearBackToTableBodyContext() {
			while (
				(this.currentTagName !== $$5.TBODY &&
					this.currentTagName !== $$5.TFOOT &&
					this.currentTagName !== $$5.THEAD &&
					this.currentTagName !== $$5.TEMPLATE &&
					this.currentTagName !== $$5.HTML) ||
				this.treeAdapter.getNamespaceURI(this.current) !== NS$3.HTML
			) {
		...
		tryPeekProperlyNestedBodyElement() {
			...
			return element && this.treeAdapter.getTagName(element) === $$5.BODY
				? element
				: null;
		}
	...
	class FormattingElementList$1 {
		...
		getElementEntry(element) {
			for (let i = this.length - 1; i >= 0; i--) {
				...
				if (
					entry.type === FormattingElementList$1.ELEMENT_ENTRY &&
	...
	class LocationInfoTokenizerMixin$2 extends Mixin$7 {
		constructor(tokenizer) {
			...
			this.posTracker = Mixin$7.install(
				tokenizer.preprocessor,
				PositionTrackingPreprocessorMixin$1
			);
			...
			if (loc) {
				if (closingToken.location) {
					...
					const isClosingEndTag =
						closingToken.type === Tokenizer$2.END_TAG_TOKEN &&
		...
		_getOverriddenMethods(mxn, orig) {
			return {
				_bootstrap(document, fragmentContext) {
					...
					const tokenizerMixin = Mixin$5.install(
						this.tokenizer,
						LocationInfoTokenizerMixin$1
					);
				...
				_processToken(token) {
					...
					const requireExplicitUpdate =
						...
						(token.tagName === $$4.HTML ||
							(token.tagName === $$4.BODY &&
								this.openElements.hasInScope($$4.BODY)));
				...
				_insertCharacters(token) {
					...
					const hasFosterParent =
						this._shouldFosterParentOnInsertion();

					const parent =
						(hasFosterParent &&
							mxn.lastFosterParentingLocation.parent) ||
	...
	class ErrorReportingPreprocessorMixin$1 extends ErrorReportingMixinBase$2 {
		constructor(preprocessor, opts) {
			...
			this.posTracker = Mixin$3.install(
				preprocessor,
	...
	class ErrorReportingTokenizerMixin$1 extends ErrorReportingMixinBase$1 {
		constructor(tokenizer, opts) {
			...
			const preprocessorMixin = Mixin$2.install(
				tokenizer.preprocessor,
	...
	//Tree mutation
	const appendChild$1 = (_default.appendChild = function (
		parentNode,
		newNode
	) {
	...
	const insertBefore$2 = (_default.insertBefore = function (
		parentNode,
		newNode,
		referenceNode
	) {
	...
	const SVG_TAG_NAMES_ADJUSTMENT_MAP =
		(foreignContent$1.SVG_TAG_NAMES_ADJUSTMENT_MAP = {
			altglyph: "altGlyph",
			altglyphdef: "altGlyphDef",
			altglyphitem: "altGlyphItem",
			animatecolor: "animateColor",
	...
	foreignContent$1.causesExit = function (startTagToken) {
		...
		const isFontWithAttrs =
			...
			(Tokenizer$1.getTokenAttr(startTagToken, ATTRS$1.COLOR) !== null ||
				Tokenizer$1.getTokenAttr(startTagToken, ATTRS$1.SIZE) !==
					null ||
	...
	function isHtmlIntegrationPoint(tn, ns, attrs) {
		if (ns === NS$2.MATHML && tn === $$3.ANNOTATION_XML) {
		...
		return (
			ns === NS$2.SVG &&
			(tn === $$3.FOREIGN_OBJECT || tn === $$3.DESC || tn === $$3.TITLE)
		);
	}

	foreignContent$1.isIntegrationPoint = function (tn, ns, attrs, foreignNS) {
		if (
			(!foreignNS || foreignNS === NS$2.HTML) &&
		...
		if (
			(!foreignNS || foreignNS === NS$2.MATHML) &&
	...
	class Parser$1 {
		...
		_shouldProcessTokenInForeignContent(token) {
			...
			if (
				this.treeAdapter.getTagName(current) === $$2.ANNOTATION_XML &&
				ns === NS$1.MATHML &&
			...
			const isMathMLTextStartTag =
				token.type === Tokenizer.START_TAG_TOKEN &&
				token.tagName !== $$2.MGLYPH &&
				token.tagName !== $$2.MALIGNMARK;

			if (
				(isMathMLTextStartTag || isCharacterToken) &&
				this._isIntegrationPoint(current, NS$1.MATHML)
			) {
				return false;
			}
		...
		_findFosterParentingLocation() {
			...
			for (let i = this.openElements.stackTop; i >= 0; i--) {
				...
				if (tn === $$2.TEMPLATE && ns === NS$1.HTML) {
					location.parent =
						this.treeAdapter.getTemplateContent(openElement);
					break;
				} else if (tn === $$2.TABLE) {
					location.parent =
						this.treeAdapter.getParentNode(openElement);
	...
	function aaInsertLastNodeInCommonAncestor(p, commonAncestor, lastElement) {
		if (p._isElementCausesFosterParenting(commonAncestor)) {
			...
			if (tn === $$2.TEMPLATE && ns === NS$1.HTML) {
				commonAncestor =
					p.treeAdapter.getTemplateContent(commonAncestor);
			}
	...
	function doctypeInInitialMode(p, token) {
		...
		const mode = token.forceQuirks
			? HTML$1.DOCUMENT_MODE.QUIRKS
			: doctype$2.getDocumentMode(token);
	...
	function aStartTagInBody(p, token) {
		const activeElementEntry =
			p.activeFormattingElements.getElementEntryInScopeWithTagName($$2.A);
	...
	function tableStartTagInBody(p, token) {
		if (
			p.treeAdapter.getDocumentMode(p.document) !==
				HTML$1.DOCUMENT_MODE.QUIRKS &&
			p.openElements.hasInButtonScope($$2.P)
		) {
			p._closePElement();
		}
	...
	function formEndTagInBody(p) {
		...
		if (
			...
			p.openElements.hasInScope($$2.FORM)
		) {
			p.openElements.generateImpliedEndTags();
	...
	function endTagInTableBody(p, token) {
		...
		if (tn === $$2.TBODY || tn === $$2.TFOOT || tn === $$2.THEAD) {
			...
			(tn !== $$2.BODY &&
				...
				tn !== $$2.COL &&
				tn !== $$2.COLGROUP) ||
			(tn !== $$2.HTML && tn !== $$2.TD && tn !== $$2.TH && tn !== $$2.TR)
		) {
	...
	function endTagInRow(p, token) {
		...
		if (tn === $$2.TR) {
			...
			if (
				...
				p.openElements.hasInTableScope($$2.TR)
			) {
				p.openElements.clearBackToTableRowContext();
				p.openElements.pop();
				p.insertionMode = IN_TABLE_BODY_MODE;
	...
	function startTagInCell(p, token) {
		...
		if (
			...
			if (
				p.openElements.hasInTableScope($$2.TD) ||
				p.openElements.hasInTableScope($$2.TH)
			) {
				p._closeTableCell();
				p._processToken(token);
			}
	...
	function endTagInSelect(p, token) {
		...
		if (tn === $$2.OPTGROUP) {
			...
			if (
				p.openElements.currentTagName === $$2.OPTION &&
				prevOpenElementTn === $$2.OPTGROUP
			) {
				p.openElements.pop();
			}
			...
			tn === $$2.SELECT &&
			p.openElements.hasInSelectScope($$2.SELECT)
		) {
			p.openElements.popUntilTagNamePopped($$2.SELECT);
			p._resetInsertionMode();
		} else if (tn === $$2.TEMPLATE) {
	...
	function endTagInFrameset(p, token) {
		if (
			...
			if (
				!p.fragmentContext &&
				p.openElements.currentTagName !== $$2.FRAMESET
			) {
	...
	function endTagInForeignContent(p, token) {
		for (let i = p.openElements.stackTop; i > 0; i--) {
			...
			if (
				p.treeAdapter.getTagName(element).toLowerCase() ===
				token.tagName
			) {
				p.openElements.popUntilElementPopped(element);
				break;
	...
	class Serializer$1 {
		...
		_serializeChildNodes(parentNode) {
			...
			if (childNodes) {
				for (
					let i = 0, cnLength = childNodes.length;
					i < cnLength;
					i++
				) {
					...
					if (this.treeAdapter.isElementNode(currentNode)) {
						...
						this.treeAdapter.isDocumentTypeNode(currentNode)
					) {
		...
		_serializeDocumentTypeNode(node) {
			...
			this.html +=
				"<" + doctype$1.serializeContent(name, null, null) + ">";
		}
	}
	...
	lib$1.parseFragment = function parseFragment(
		fragmentContext,
		html,
		options
	) {
	...
	const insertBefore$1 = (lib.insertBefore = function (
		parentNode,
		newNode,
		referenceNode
	) {
	...
	function parse$4(content, options, isDocument) {
		var opts = {
			scriptingEnabled:
		...
		for (var index = 0; index < nodes.length; index += 1) {
			...
			if (domhandler_1$4.isDocument(node)) {
				(_a = Array.prototype.splice).call.apply(
					_a,
					tslib_1$5.__spreadArray([nodes, index, 1], node.children)
				);
			}
		}
	...
	function html$1(dom, options) {
		 ...
		 */
		var opts = tslib_1$4.__assign(
			tslib_1$4.__assign(
				tslib_1$4.__assign({}, options_1$2.default),
				this ? this._options : {}
			),
			options_1$2.flatten(
	 ...
	 */
	function xml(dom) {
		var options = tslib_1$4.__assign(
			tslib_1$4.__assign({}, this._options),
			{ xmlMode: true }
		);
	...
	function text$1(elements) {
		...
		for (var i = 0; i < elems.length; i++) {
			var elem = elems[i];
			if (htmlparser2_1$4.DomUtils.isText(elem)) ret += elem.data;
			else if (
				htmlparser2_1$4.DomUtils.hasChildren(elem) &&
	 ...
	 * Parser
	 */
	function parse$2(content, options, isDocument) {
	 ...
	 */
	utils$2.isTag = htmlparser2_1$2.DomUtils.isTag;
	/**
					...
					if (utils_1$5.isTag(el))
						setProp(
							...
							value.call(
								el,
								i,
								getProp(el, name, _this.options.xmlMode)
							),
	...
	function readData(el, name) {
		...
		if (name == null) {
			...
			jsNames = domNames.map(function (domName) {
				return utils_1$5.camelCase(
					domName.slice(dataAttrPrefix.length)
				);
			});
		} else {
	 ...
	 */
	traversing.parentsUntil = _matchUntil(
		function (_a) {
			var parent = _a.parent;
			return parent && !domhandler_1$1.isDocument(parent) ? parent : null;
		},
	...
	traversing.children = _matcher(function (elem) {
		return htmlparser2_1$1.DomUtils.getChildren(elem).filter(
			utils_1$4.isTag
		);
	}, _removeDuplicates);
	/**
	...
	function contents() {
		var elems = this.toArray().reduce(function (newElems, elem) {
			return domhandler_1$1.hasChildren(elem)
	...
	function has(selectorOrHaystack) {
		...
		return this.filter(
			typeof selectorOrHaystack === "string"
				...
				: function (_, el) {
						return (
							_this._make(el).find(selectorOrHaystack).length > 0
						);
				  }
		);
	}
	...
	function add(other, context) {
		...
		var contents = uniqueSort(
			tslib_1$3.__spreadArray(
				tslib_1$3.__spreadArray([], this.get()),
				selection.get()
			)
		);
	...
	manipulation.clone =
		...
		manipulation.after =
		manipulation.wrapAll =
		manipulation.unwrap =
		manipulation.wrapInner =
		manipulation.wrap =
		manipulation.prepend =
		manipulation.append =
		manipulation.prependTo =
		manipulation.appendTo =
		manipulation._makeDomArray =
			void 0;
	...
	var parse_1$2 = tslib_1$2.__importStar(parse$3);
	...
	function appendTo(target) {
		var appendTarget = utils_1$3.isCheerio(target)
			? target
			: this._make(target);
	...
	function prependTo(target) {
		var prependTarget = utils_1$3.isCheerio(target)
			? target
			: this._make(target);
	...
	function html(str) {
		...
		return utils_1$3.domEach(this, function (el) {
			...
			var content = utils_1$3.isCheerio(str)
				? str.toArray()
				: parse_1$2.default("" + str, opts, false).children;
	...
	function text(str) {
		...
		if (typeof str === "function") {
			...
			return utils_1$3.domEach(this, function (el, i) {
				text.call(
					_this._make(el),
					str.call(el, i, static_1.text([el]))
				);
			});
		}
	...
	function getCss(el, prop) {
		...
		var styles = parse$1(el.attribs.style);
		if (typeof prop === "string") {
			return styles[prop];
		}
	...
	var parse_1$1 = tslib_1$1.__importDefault(parse$3);
	...
	var Cheerio = /** @class */ (function () {
		...
		function Cheerio(selector, context, root, options) {
			...
			if (root) {
				...
				this._root = new this.constructor(
					root,
					null,
					null,
					this.options
			...
			var searchContext = !context
				...
				? utils_1.isHtml(context)
					? // $('li', '<ul>...</ul>')
					  this._make(
							parse_1$1.default(context, this.options, false)
					  )
					: // $('li', 'ul')
	...
	var parse_1 = tslib_1.__importDefault(parse$3);
	/**
	...
	(function (module, exports) {
		...
		function stringify(ast, options) {
			...
			var css = reduce(ast.stylesheet.rules, stringifyNode)
				.join("\n")
				.trim();
		...
		function reduce(items, fn) {
			return items.reduce(function (results, item) {
				var result =
					item.type === "comment" ? stringifyComment(item) : fn(item);
	...
	*/
	...
	var replacer = function (
		...
		if (tagName) {
			...
			(current.pseudos || (current.pseudos = [])).push({
				...
				escapedValue: pseudoClassValue
					? escape(pseudoClassValue)
					: null,
			});
	...
	var parse = function (expression) {
		...
		return (
			cache[expression] ||
			(cache[expression] = new Expressions(expression))
		);
	};
	...
	(function (exports) {
		...
		var mensch$1 = mensch;
		var Selector = selector.exports;
		var Property = property.exports;
		...
		exports.parseCSS = function (css) {
			var parsed = mensch$1.parse(css, {
				position: true,
				comments: true,
			});
			var rules =
				typeof parsed.stylesheet != "undefined" &&
		...
		exports.getPreservedText = function (css, options, ignoredPseudos) {
			...
			for (var i = rules.length - 1; i >= 0; i--) {
				if (
					(options.fontFaces && rules[i].type === "font-face") ||
		...
		var createEntityConverters = function () {
			...
			var encodeCodeBlocks = function (html) {
				...
				Object.keys(blocks).forEach(function (key) {
					var re = new RegExp(
						blocks[key].start + "([\\S\\s]*?)" + blocks[key].end,
						"g"
					);
	...
	var inline = function makeJuiceClient(juiceClient) {
		...
		function inlineDocument($, css, options) {
			...
			if (options.insertPreservedExtraCss && options.extraCss) {
				...
				if (preservedText) {
					...
					$appendTo
						.first()
						.append("<style>" + preservedText + "</style>");
				}
			}
			...
			function handleRule(rule) {
				...
				for (var i = 0; i < parsedSelector.length; ++i) {
					...
					if (subSel.pseudos) {
						for (var j = 0; j < subSel.pseudos.length; ++j) {
							...
							if (
								juiceClient.ignoredPseudos.indexOf(
									subSelPseudo.name
								) >= 0
							) {
								return;
				...
				els.each(function () {
					...
					if (
						el.name &&
						juiceClient.nonVisualElements.indexOf(
							el.name.toUpperCase()
						) >= 0
					) {
						return;
					...
					if (!el.styleProps) {
						...
						if ($(el).attr(styleAttributeName)) {
							var cssText =
								"* { " + $(el).attr(styleAttributeName) + " } ";
							addProps(
								utils.parseCSS(cssText)[0][1],
								new utils.Selector("<style>", true)
							);
						}
					...
					if (!el.counterProps) {
						el.counterProps =
							el.parent && el.parent.counterProps
					...
					function resetCounter(el, value) {
						...
						for (var j = 0; j < tokens.length; j++) {
							...
							isNaN(resetval)
								? (el.counterProps[counter] = counters[
										counter
								  ] =
										0)
								: (el.counterProps[counter] = counters[
					...
					function addProps(style, selector) {
						for (var i = 0, l = style.length; i < l; i++) {
							if (style[i].type == "property") {
								...
								var additionalPriority = [
									style[i].position.start.line,
									style[i].position.start.col,
								];
								var prop = new utils.Property(
									name,
									value,
									selector,
									important ? 2 : 0,
								...
								if (
									juiceClient.excludedProperties.indexOf(
										name
									) < 0
								) {
									if (
										(existing &&
											existing.compare(prop) === prop) ||
										!existing
									) {
				...
				if (juiceClient[dimension + "Elements"].indexOf(elName) > -1) {
					for (var i in el.styleProps) {
						if (el.styleProps[i].prop === dimension) {
							...
							if (
								juiceClient.tableElements.indexOf(elName) >
									-1 &&
								value.match(/\%/)
							) {
		...
		function parseContent(el) {
			...
			for (var i = 0; i < tokens.length; i++) {
				...
				var varMatch = tokens[i].match(
					/var\s*\(\s*(.*?)\s*(,\s*(.*?)\s*)?\s*\)/i
				);
				if (varMatch) {
					var variable =
						findVariableValue(el, varMatch[1]) || varMatch[2];
					parsed.push(variable.replace(/^['"]|['"]$/g, ""));
					continue;
				}

				var counterMatch = tokens[i].match(
					/counter\s*\(\s*(.*?)\s*(,\s*(.*?)\s*)?\s*\)/i
				);
		...
		function getStylesData($, options) {
			...
			stylesList.each(function () {
				...
				styleDataList = usingParse5
					? styleElement.childNodes
				...
				if (
					...
					$(styleElement).attr("data-embed") === undefined
				) {
				...
				if (
					...
					var text = usingParse5
						? styleElement.childNodes[0].nodeValue
						: styleElement.children[0].data;
					var preservedText = utils.getPreservedText(
						text,
						{
							mediaQueries: options.preserveMediaQueries,
							fontFaces: options.preserveFontFaces,
	...
	juiceClient.inlineContent = function (html, css, options) {
		return cheerio(
			html,
			{ xmlMode: options && options.xmlMode },
			juiceClient.inlineDocument,
			[css, options]
		);
	};

	var client = juiceClient;
	var rawHTML = $("#emailcontents").val();
	var result = client(rawHTML);
	$("#emailcontents").val(result);

	// function inlineHTML() {
	// 	console.log("I am in my function");
	// 	var result = client("<style>div{color:red;}</style><div/>");
	// 	console.log(result);
	// }

	// $("body").append('<button class="inlineHTML">Inline HTML</button>');
	// $(".inlineHTML").on("click", function () {
	// 	inlineHTML();
	// });
})();
